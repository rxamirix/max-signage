...
export interface SonarGridProps extends React.ComponentProps<"div"> {
  ...
  color?: string;
  /**
   * Brand pair for a live wavefront. Near the ping the ring is `colors[0]`;
   * as it expands it interpolates to `colors[1]`. Falls back to `color` when omitted.
   */
  ...
  /**
   * How quickly the wavefront turns from colors[0] to colors[1], as a fraction of
   * the shorter canvas side. Smaller = navy arrives sooner.
   */
  colorReach?: number;
  /** Seconds a ring stays visible. It fades out in place instead of running off the canvas. */
  fadeDuration?: number;
}
...
export function SonarGrid({
  ...
  React.useEffect(() => {
    ...
    const addRing = (x: number, y: number, born: number) => {
      ...
      rings.push({ x, y, born, reverse: pingFlip });
      pingFlip = !pingFlip;
    ...
    const draw = (now: number) => {
      ...
      const lifetime = Math.max(0.6, o.fadeDuration);
      ...
      const live = ringsRef.current.map((r) => {
        ...
        const fade = progress < 0.42 ? 1 : 1 - (progress - 0.42) / 0.58;
        return {
          x: r.x,
          y: r.y,
          radius,
      ...
      for (let i = 0; i < cols; i++) {
        ...
        for (let j = 0; j < rows; j++) {
          ...
          if (energy < 0.01) {
            ...
            const from = originReverse ? colorB : colorA;
            const to = originReverse ? colorA : colorB;
            hot.push(cx, cy, energy);
            hotColors.push(mixHex(from, to, originMix));
          }
        }
      }
      ctx.fill();

      // Pass 2: only the dots on a wavefront get their own alpha, radius, and brand colour.
      for (let k = 0, n = 0; k < hot.length; k += 3, n++) {
        const energy = hot[k + 2] ?? 0;
        ctx.fillStyle = hotColors[n] ?? stroke;
    ...
    const onDown = (e: PointerEvent) => {
      ...
      if (target instanceof Element && target.closest("header, a, button, input, textarea, select, label")) return;
      const rect = host.getBoundingClientRect();
  ...
  React.useEffect(() => {
    refreshRef.current();
  }, [spacing, dotRadius, baseOpacity, color, colors, pingEvery, speed, ringWidth, amplitude, interactive, maxRings, pingArea, colorReach, fadeDuration]);
